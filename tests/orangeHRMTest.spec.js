...
test("TC_004 - Create User", async ({ page }) => {
  const loginPage = new sections.Login(page, test);
  await loginPage.launchMainUrl();
  await loginPage.loginApplication(
    [process.env.BASE_USERNAME],
  ...
  await adminPage.setUserCredentials(
    ...
    [data.create_user.employee_name],
...
test("TC_005 - Make user an Admin", async ({ page }) => {
  const loginPage = new sections.Login(page, test);
  await loginPage.launchMainUrl();
  await loginPage.loginApplication(
    [process.env.BASE_USERNAME],
  ...
  const adminPage = new sections.AdminPage(page, test);
  await adminPage.navigateToAdminPortal();
  await expect(
    page,
...
test("TC_006 - Validating Buzz Interface", async ({ page }) => {
  const loginPage = new sections.Login(page, test);
  await loginPage.launchMainUrl();
  await loginPage.loginApplication(
    [process.env.BASE_USERNAME],
...
test("TC_007 - Comments scenario in Buzz Interface", async ({ page }) => {
  const loginPage = new sections.Login(page, test);
  await loginPage.launchMainUrl();
  await loginPage.loginApplication(
    [process.env.BASE_USERNAME],
  ...
  await page.waitForTimeout(parseInt(process.env.SMALL_WAIT));
  await expect(buzzPage.commentText).not.toBeVisible();
  await page.goBack();
  const homePage = new sections.HomePage(page, test);
  await homePage.logOut();
});
